# pygame-Search-the-Key

[![Join the chat at https://gitter.im/pygame-Search-the-Key/Lobby](https://badges.gitter.im/pygame-Search-the-Key/Lobby.svg)](https://gitter.im/pygame-Search-the-Key/Lobby?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge&utm_content=badge) [![forthebadge made-with-python](http://ForTheBadge.com/images/badges/made-with-python.svg)](https://www.python.org/)
[![KWoC 2018](https://img.shields.io/badge/KWoC-2018-0078D6.svg?style=flat-square&longCache=true&logo=data:image/png;base64,)](https://kwoc.kossiitkgp.org/)

Table of contents
Introduction 
Requirements 
Working
Installation guide 
Screenshot 
pygame-running 
License

INTRODUCTION

A small game made using pygame. Search the key from a 5x5 grid in 6 turns with indications on how 'hot' or 'cold' they are with respect to the location of the key.

PREREQUISITES

1.Python (2 or 3)

2.Pygame

HOW IT WORKS 

OBJECTIVE OF THE GAME

A very simple objective of finding the key before your turns get over or else you lose. 

INSTALLATION GUIDE

SETUP INSTRUCTIONS
STEP 1: 

Fork the repository and clone your fork into your machine
Go to the command prompt/terminal. Use

`git clone https://github.com/<insert your username>/pygame-Search-the-Key.git`.

This will create a clone of the project where you can make changes as you desire.


RUNNING INSTRUCTIONS
STEP 2: 

Running the Game
Type `python main.py` to run the game.(Ensure is python in your path)


(https://github.com/vineetjc/pygame-Search-the-Key/blob/master/screenshots/Capture.png)


![Alt text](https://github.com/vineetjc/pygame-Search-the-Key/blob/master/screenshots/Cpture2.png)

LICENSE
The code in this project is licensed under the MIT License 2017 - Vineet Jiji Cherian.

